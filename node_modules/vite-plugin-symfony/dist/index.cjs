...
// ../../node_modules/.pnpm/tsup@8.3.5_postcss@8.4.49_typescript@5.7.2/node_modules/tsup/assets/cjs_shims.js
...
function resolveOutDir(unknownBase) {
  const baseURL = new URL(unknownBase, importMetaUrl);
  const base = baseURL.protocol === "file:" ? unknownBase : baseURL.pathname;
  const publicDirectory = "public";
  return (0, import_node_path2.join)(publicDirectory, trimSlashes(base));
}
  ...
  return {
    ...
    configureServer(devServer) {
      ...
      devServer.httpServer?.once("listening", () => {
        if (viteConfig.env.DEV && !import_node_process2.default.env.VITEST) {
      ...
      devServer.middlewares.use(function symfonyInternalsMiddleware(req, res, next) {
        ...
        if (req.url === import_node_path3.default.posix.join(viteConfig.base, INFO_PUBLIC_PATH)) {
          res.statusCode = 200;
      ...
      if (pluginOptions.servePublic !== false) {
        const serve = (0, import_sirv.default)(pluginOptions.servePublic, {
          dev: true,
          etag: true,
          extensions: [],
...
function parseStimulusRequest(code, moduleId, pluginOptions, viteConfig) {
  ...
  if (!id) {
    const relativePath = (0, import_node_path4.relative)(viteConfig.root, filePath);
...
function isPathIncluded(basePath, targetPath) {
  const normalizedBasePath = (0, import_node_path5.resolve)(basePath);
  const normalizedTargetPath = (0, import_node_path5.resolve)(targetPath);
  const basePathWithSep = normalizedBasePath.endsWith(import_node_path5.sep) ? normalizedBasePath : normalizedBasePath + import_node_path5.sep;
...
function symfonyStimulus(pluginOptions, logger) {
  ...
  return {
    ...
    async configResolved(config) {
      viteConfig = config;
      controllersFilePath = (0, import_node_path6.resolve)(viteConfig.root, pluginOptions.controllersFilePath);
      try {
        await (0, import_promises.stat)(controllersFilePath);
        controllersJsonContent = JSON.parse((await (0, import_promises.readFile)(controllersFilePath)).toString());
      } catch {
    ...
    transform(code, id, options) {
      ...
      if (viteCommand === "serve") {
        ...
        if (isPathIncluded(viteConfig.root, id)) {
          const relativePath = (0, import_node_path6.relative)(viteConfig.root, id);
